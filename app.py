...
def view_logs():
    ...
    cursor.execute(
        "SELECT id, name, date, time, emotion, gender FROM attendance_logs ORDER BY id DESC"
    )

    records = cursor.fetchall()

    cursor.execute(
        "SELECT COUNT(DISTINCT name) FROM attendance_logs"
    )
    ...
    cursor.execute(
        "SELECT COUNT(DISTINCT name) FROM attendance_logs WHERE date=?",
        (today,)
    )
...
def percentage():
    ...
    for student in all_students:
        ...
        attended = attendance_map.get(name, 0)
        ...
        students.append({
            ...
            "status": status,
            "needed_classes": max(0, needed_classes)
        })
